...
const getTemplateList = dispatch => async ({ googleId }) => {
  try {
    // Test endpoint
    ...
    dispatch({ type: 'GET_TEMPLATE_ERR', payload: err.response });
  }
};

const addTemplate = dispatch => async (data, { googleId }) => {
  ...
  try {
    ...
    dispatch({
      ...
      payload: await getTemplateList(dispatch)({ googleId })
    });
  } catch (err) {
    dispatch({ type: 'GET_TEMPLATE_ERR', payload: err.response });
  }
...
const deleteTemplate = dispatch => async (id, { googleId }) => {
  try {
    // Test endpoint
    await axios.delete(
...
const useTemplate = () => {
  ...
  return {
    ...
    getTemplateList: getTemplateList(dispatch),
    addTemplate: addTemplate(dispatch),
    deleteTemplate: deleteTemplate(dispatch)
  };
};
